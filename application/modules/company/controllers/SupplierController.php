...
class Company_SupplierController extends Dataservice_Controller_Action
{    
    public function init()
    {
	$this->view->headScript()->appendFile("/javascript/company/supplier.js");
	parent::init();
    }
    ...
    {
	$Supplier   = $this->getEntityFromParamFields("Company\Supplier", array("id"));
	$form	    = new Forms\Company\Supplier(array("method" => "post"), $Supplier);
	...
	if($this->isPostAndValid($form))
	{
	    try 
	    {
		$supplier_data	= $this->_params["company_supplier"];
	...
	$form = new Forms\Company\Supplier\AddCompany(array("method" => "post"));
	
	if($this->isPostAndValid($form))
	{
	    try 
	    {
		$Company = $this->_em->find("Entities\Company", $this->_params["company_supplier_addcompany"]["company_id"]);
